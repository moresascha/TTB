...
#if 0
void SortEvents(dpEventLines* eventLine);
...
__device__ CTreal msgBuffer[512];
...
struct MemorySizes
{
    ...
    {
        __ct_printf("\nMemorySizes: eventMemory=%d nodeMemory=%d perLevelNodeMemory=%d leafNodeMemory=%d leafContentMemory=%d \n",
            eventMemory, nodeMemory, perLevelNodeMemory, leafNodeMemory, leafContentMemory);
    }
};
...
void dpEventLines::Resize(size_t newSize, cudaStream_t pStream)
{
    ...
    for(int i = 0; i < 3; ++i)
    {
        eventLines[i].Resize((CTuint)(newSize));
    }
...
__device__ void dpScanBinaryTriples(ConstTuple<3, T>& src, Tuple<3, CTuint>& scanned, Tuple<3, CTuint>& sums, CTuint N, Operator op)
{
    ...
    binaryTripleGroupScanNoOpt<block><<<grid, block>>>(
        src, scanned, sums, op,
        N);
    ...
    if(sumsCount > 1)
    {
        ...
        completeScan2<1024, 3><<<3, 1024>>>(constSums, sums, _op, sumsCount);
...
#ifndef DEBUG_CUDA
__device__ void dpCcomputeSAH_Splits(CTuint nodeCount, CTuint eventCount)
{
...
//         {
//             CTuint length = 2 * g_nodes.contentCount[i];
//             
...
//             uint grid = nutty::cuda::GetCudaGrid((uint)length, elementsPerBlock);
// 
//             IndexedSAHSplit neutral;
//             neutral.index = 0;
//             neutral.sah = FLT_MAX;
//             ReduceIndexedSplit op;
//             if(grid > 1)//d > elementsPerBlock)
//             {
//                 optblockReduce<blockSize><<<grid, blockSize>>>(g_splits.indexedSplit + start, g_splits.indexedSplit + start, op, neutral, elementsPerBlock, length);
//                 optblockReduce<blockSize><<<1, blockSize>>>(g_splits.indexedSplit + start, g_splits.indexedSplit + start, op, neutral, grid, grid);
//             }
//             else
//             {
//                 optblockReduce<blockSize><<<1, blockSize>>>(g_splits.indexedSplit + start, g_splits.indexedSplit + start, op, neutral, length, length);
//             }
// 
//             start += length;
//         }
...
//                 msgBuffer[1] = d_memorySizes.perLevelNodeMemory;
            ...
            for(int offset = 0; offset < childCount; offset += blockDim.x)
            {
                ...
                if(id < childCount)
                {
                    ...
                    d_maskedInteriorContent[id] = /*(mask < 2) */ (mask == 0) * g_nodes.contentCount[id];
                }
            }
...
//             }
        ...
        currentLeafCount += leafCount;
            ...
            if(threadIdx.x == 0)
            {
                ...
                msgBuffer[10*d+4] = (CTreal)interiorPrimCount;
                msgBuffer[10*d+5] = (CTreal)currentLeafCount;
                msgBuffer[10*d+6] = (CTreal)-2;
...
//         GrowNodeMemory(8192);
//         GrowEventMemory(279948);
//         GrowLeafNodeMemory(41434);
//         GrowLeafContentMemory(3504132);
#else
        //angel
//         GrowPerLevelNodeMemory(23040);
//         GrowNodeMemory(32768);
//         GrowEventMemory(1899984);
//         GrowLeafNodeMemory(10980);
//         GrowLeafContentMemory(1899984);

        //dragon
//         GrowPerLevelNodeMemory(21128);
//         GrowNodeMemory(131072);
//         GrowEventMemory(3491628);
//         GrowLeafNodeMemory(40130);
//         GrowLeafContentMemory(3491628);
...
//         GrowLeafContentMemory(4 * primitiveCount);
    ...
    DEVICE_SYNC_CHECK();
        SortEvents(&m_eventLines);
    DEVICE_SYNC_CHECK();
        cudaReorderEvent3(2 * primitiveCount, 2 * elementGrid, elementBlock, rawEvents, rawEventkeys, m_pStream);
    DEVICE_SYNC_CHECK();
        dpTreeData data;
        ...
        //CTbyte one = 1;
        //cudaMemcpyToSymbolAsync(d_toggleIndex, &one, 1, 0, cudaMemcpyHostToDevice, m_pStream);

        const CTuint BLOCK_SIZE = 128;
        dpCreateKDTree<BLOCK_SIZE><<<1, BLOCK_SIZE, 0, m_pStream>>>(data.eventCount, m_depth, 1,1,0,0,0,1,0);
#ifdef MEM_CHECK
        cudaError_t er = cudaDeviceSynchronize();
        if(er != cudaSuccess)
        {
            __ct_printf("%s\n", cudaGetErrorString(er)); exit(0);
        }
#endif
        ...
        for(int i = 0; i < sizeof(h_msgBuffer)/sizeof(CTuint); ++i)
        {
        ...
        if(needMoreMemory)
        {
            __ct_printf("Need more Memory: (%d %d %d %d %d)\n", h_needMoreMemory[0], h_needMoreMemory[1], h_needMoreMemory[2], h_needMoreMemory[3], h_needMoreMemory[4]);
        }
...
void cudpKDTree::GrowLeafNodeMemory(CTuint newSize)
{
    if(newSize == (CTuint)-1)
    {
        ...
        __ct_printf("posleafCount=%d currentLeafCount=%d\n\n", leafCount, currentLeafCount);
    }
...
{
    newSize = (CTuint)(newSize == -1? m_nodes_IsLeaf.Size() * 4 : newSize);
...
void cudpKDTree::GrowEventMemory(CTuint eventCount)
{
    ...
    h_memorySizes.eventMemory = 2*eventCount;
    copyMemorySizesToDevice(m_pStream);
...
template<CTuint BLOCK_SIZE>
__global__ void dpCreateKDTreeOld(CTuint eventCount, CTuint maxDepth)
{
#ifndef DEBUG_CUDA
    CTuint nodesCount = 1;
...
//                 msgBuffer[1] = d_memorySizes.perLevelNodeMemory;
            ...
            for(int offset = 0; offset < childCount; offset += blockDim.x)
            {
                ...
                {
                    CTclipMask_t mask = d_activeNodesIsLeaf[nodesCount + id];
                    d_maskedInteriorContent[id] = (1 ^ mask) * g_nodes.contentCount[id];
                }
            }
